...
use std::{path::{PathBuf, Path}, fs::{DirEntry, self}};
...
// ([-+]?[0-9]+(\.[0-9]*)?([eE][-+]?[0-9]+)?)(M)?
    ...
    fn read_files(dirs: Vec<&str>) -> Vec<Result<PathBuf, (PathBuf, pest::error::Error<Rule>)>> {
        let dirs: Vec<_>  = dirs.into_iter()
        .filter(|x| x.ends_with("edn"))
        .collect();
...
mod tests {
    ...
    use super::*;
    const VALID_FOLDERS: [&str; 4] = ["./test", "./examples/edn-tests/valid-edn","./examples/edn-tests/performance","./examples/antlr-grammars-v4/edn/examples"];
    const INVALID_FOLDERS: [&str; 1] = ["./examples/edn-tests/invalid-edn"];

    #[test]
    fn test_edn_files() {
        let a = read_files(VALID_FOLDERS.into()).into_iter()
        .filter_map(|x| x.err())
        .map(|x| dbg!(x.0))
        .count();
        assert_eq!(a, 0);
    ...
    fn test_invalid_edn_files() {
        let a = read_files(INVALID_FOLDERS.into()).into_iter()
        .filter_map(|x| x.ok())
        .map(|x| dbg!(x))
        .count();
        assert_eq!(a, 0);
